...
#        print "counter is {0}\n".format(global_counter)
        print 'time_cost:\t%2.4f\n'%(te-ts)
#        if dga_flag =='0':
            
##        with open('performance_result.txt', 'a') as file:
#            if counter > 1000002 and counter <= 2000002:
#                file.write('1:\t%2.4f\n'%(te-ts))
#            elif counter > 2000002:
#                file.write('2:\t%2.4f\n'%(te-ts))
#            else:
##                file.write('time_cost:\t%2.4f\n'%(te-ts))
        return (result,(te-ts))
...
#    global all
    ts = time.time()
    with open('one_url.txt','rb') as input_file:
      with open('one_url.csv', 'w') as csvoutput:
        counter = 0

        writer = csv.writer(csvoutput, lineterminator='\n')
        rows = pool.map(destributed_row_processing,input_file)

        header = ['url', 'domain','family','breakdowns_str', 'pattern_str','domain_length','total_word_count', 'mea_word_count_enchant',
            'mea_word_ratio_enchant',
            'mea_char_ratio_enchant',
            'mea_word_count_google',
            'mea_word_ratio_google',
    ...
    logging.error('Time_Record: {0} hr\n'.format((te-ts)/360))
